#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>



int main( int argc, char * argv [] ) {
    if(argc != 4){
        printf("Wrong number of arguments, Should be total_ram, mem_procent, time_in_s");
        return 1;
    }
    long total_ram = atol(argv[1])*1000;
    int mem_procent = atoi(argv[2]);
    int time_in_s = atoi(argv[3]);
    printf("%d\n",time_in_s > 0);
    if (total_ram > 0 && mem_procent > 0 && time_in_s > 0){
        long bytes_to_allocate = total_ram*mem_procent*0.01;
        char* ptr = (char*)calloc(bytes_to_allocate,sizeof(char));
        printf("%zu\n",bytes_to_allocate);
        if(ptr == NULL)                     
        {
            printf("Error! memory not allocated. malloc\n");
            exit(0);
        }
        // int ret_mlock = mlock(ptr,bytes_to_allocate);
        // if(ret_mlock <0){
        //     printf("Error! memory not allocated. mlock\n");
        //     exit(0);
        // }
        char* to_set = "";
        
        // memset(ptr,'x',bytes_to_allocate);
        for(long i = 0; i < bytes_to_allocate-1024; i += 1024)
        {
            strcpy(&ptr[i], to_set);
            // printf("\ntest! %c\n",ptr[i]);
        }

        printf("\ntest! %c\n",ptr[0]);
        sleep(time_in_s);
        // munlock(ptr,bytes_to_allocate);
        free(ptr);
    } else {
        printf("Args needs to be bigger then zero, total_ram: %d, mem_procent: %d, time_in_s:%d \n",total_ram,mem_procent,time_in_s);
        return 1;   
    }
}
